...
def word2vec(x, output_folder):
    ...
    x_sequences = kprocessing.sequence.pad_sequences(tokenizer.texts_to_sequences(x), maxlen=MAX_LEN)
...
def create_sequences(x_train, x_validation, x_test, output_folder):
    print("Start sequences")
    max_words = 7000
    tokenizer = kprocessing.text.Tokenizer(lower=True, split=' ', num_words=max_words, oov_token="<pad>", filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n')
...
def main(binary_classification, dataset) :
    ...
    x_sequences = word2vec(x, output_folder)
    ...
    x, y = sm.fit_resample(x_sequences, y)
    ...
    for train_val_index, test_index in kf.split(x, y):
        ...
        create_sequences(x_train, x_validation, x_test, path)
