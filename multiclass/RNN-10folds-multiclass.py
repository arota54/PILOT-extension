...
def create_model(shape) :
    model = tf.keras.Sequential(
        [
            #tf.keras.layers.Embedding(input_dim=shape[1], output_dim=32, trainable=False),
            tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32, input_shape=(shape[0], shape[1]))),
    ...
    """x = tf.keras.layers.Embedding(input_dim=shape[1], output_dim=64)
...
loss, accuracy, val_loss, val_accuracy, predictions, y_test = runExperiment(base_dir + "multiclass/DatasetD1/rounds/")
...
plot_history(loss, accuracy, val_loss, val_accuracy, base_dir + "multiclass/DatasetD1/rounds/")
